...
const ACTION_BUTTON_ID = 'cell-action-button';
...
const InternalTextEditor = <T extends WorksheetItem>({
  ...
  const renderedActionButton = useMemo(
    ...
    [action, transformedValue, value, onBlur, cell],
  );
    ...
    <Flex alignItems="center" justifyContent="space-between">
      <StyledInput
        ...
        onBlur={(event?: React.FocusEvent<HTMLInputElement>) => {
          const isInfinityButtonFocused = event?.relatedTarget?.id === ACTION_BUTTON_ID;

          if (isInfinityButtonFocused) {
            blurEventRef.current = event;
          }

          if (!isBlurBlocked.current && !isInfinityButtonFocused) {
            onBlur(event, cell);
          }
        }}
