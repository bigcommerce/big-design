...
export const ComboBox = typedMemo(
  <T,>({
    ...
    value,
    ...props
  }: ComboBoxProps<T>): ReturnType<React.FC<ComboBoxProps<T>>> => {
    ...
    const flattenOptions = useCallback((options: ComboBoxProps<T>['options']) => {
      const isGroups = (
        options: Array<ComboBoxOptionGroup<T> | ComboBoxOption<T>>,
      ): options is Array<ComboBoxOptionGroup<T>> =>
    ...
    const selectedOption = useMemo(() => {
      return flattenedOptions.find(
        (option): option is ComboBoxOption<T> => 'value' in option && option.value === value,
      );
    ...
    const getFirstMatchingOptionIndex = (
      filteredOptions: Array<ComboBoxOption<T> | ComboBoxAction>,
    ) => {
      return filteredOptions.findIndex((option) => !option.disabled);
    };

    const handleOnSelectedItemChange = (
      changes: Partial<UseComboboxState<ComboBoxOption<T> | ComboBoxAction | null>>,
    ) => {
    ...
    const handleOnInputValueChange = ({
      inputValue,
      isOpen,
    }: Partial<UseComboboxState<ComboBoxOption<T> | ComboBoxAction | null>>) => {
    ...
    const handleStateReducer = (
      _state: UseComboboxState<ComboBoxOption<T> | ComboBoxAction | null>,
      actionAndChanges: UseComboboxStateChangeOptions<ComboBoxOption<T> | ComboBoxAction | null>,
    ) => {
